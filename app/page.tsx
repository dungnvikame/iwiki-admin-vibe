...
type FilterState = { unit: string; team: string; period: string; category: string };
...
const makeDefaultFilter = (): FilterState => ({ unit: units[0], team: "Tất cả team", period: periods[2], category: categories[0] });
...
function Articles({ factor, open, notify, focus }: { factor: number; open: (p: NonNullable<PanelData>) => void; notify: (s:string) => void; focus?: string }) {
  ...
  const articleDetail = (a: readonly [string,string,string,string,number]) => open({title:a[0],subtitle:`${a[1]} · ${a[4]} tháng chưa review`,kind:"article",stats:[{label:"Lượt đọc",value:String(214-a[4]*2),note:"Trong kỳ",tone:"blue"},{label:"Lượt lưu",value:String(48-Math.floor(a[4]/2)),note:"Tương tác",tone:"green"},{label:"Tuổi bài",value:`${a[4]} tháng`,note:a[4]>12?"Quá hạn":"Sắp đến hạn",tone:a[4]>12?"red":"amber"}],rows:[{label:"Phân loại",value:a[3],meta:"Taxonomy hiện tại"},{label:"PIC / Tác giả",value:a[2],meta:a[1]},{label:"Ngày cập nhật cuối",value:"12/06/2024",meta:`${a[4]} tháng trước`},{label:"Hạn review đề xuất",value:"Trong 7 ngày",meta:"Theo SLA"}],content:<><div className="detail-kpis"><span><b>{a[3]}</b>Phân loại</span><span><b>{a[2]}</b>PIC / Tác giả</span><span><b>{a[4]} tháng</b>Tuổi bài viết</span></div>{detailList(["Trạng thái: Cần review", "Ưu tiên: Cao", "Hành động đề xuất: Chỉ định PIC và review trong 7 ngày"])}</>});
  return <div className="tab-content">
...
function Members({ factor, open, notify }: { factor:number; open:(p:NonNullable<PanelData>)=>void; notify:(s:string)=>void }) {
  ...
  const memberDetail=(m:typeof members[number])=>open({title:m[0],subtitle:m[1],kind:"member",stats:[{label:"Bài mới",value:String(m[2]),note:"Trong kỳ",tone:"green"},{label:"Lượt đọc",value:String(m[6]),note:"Tổng đóng góp",tone:"blue"},{label:"Cần review",value:String(m[5]),note:"Được phân công",tone:Number(m[5])>3?"red":"amber"}],rows:[{label:"Đơn vị / Team",value:m[1],meta:"Phạm vi hiện tại"},{label:"Hoạt động gần nhất",value:String(m[7]),meta:"Theo log truy cập"},{label:"Trạng thái",value:String(m[8]),meta:"Theo rule hệ thống"},{label:"Bài chờ duyệt",value:`${m[3]} bài`,meta:"Trong SLA"}],content:<><div className="detail-kpis"><span><b>{m[2]}</b>Bài mới</span><span><b>{m[6]}</b>Lượt đọc</span><span><b>{m[7]}</b>Hoạt động gần nhất</span></div>{detailList([`Trạng thái: ${m[8]}`, `${m[5]} bài cần review`, `${m[3]} bài đang chờ duyệt`])}</>});
...
export default function Home() {
  ...
  return <div className="app-shell">
    <aside className={sidebarOpen?"sidebar open":"sidebar"}><div className="brand"><span className="brand-mark">iW</span><div><strong>iWiki</strong><small>Knowledge Admin</small></div><button className="close-menu" onClick={()=>setSidebarOpen(false)}>×</button></div><nav><span className="nav-label">Knowledge Dashboard</span>{tabs.map(item=><button key={item.id} className={tab===item.id?"active":""} onClick={()=>navigate(item.id)}><Icon tone={tab===item.id?"blue":"slate"}>{item.icon}</Icon>{item.label}</button>)}<button onClick={()=>setPanel({title:"Thống kê tìm kiếm",subtitle:"30 ngày gần nhất",content:detailList(["1.284 phiên tìm kiếm", "79% tìm kiếm thành công", "66 phiên không có kết quả", "Top keyword: onboarding"])})}><Icon tone="slate">⌕</Icon>Thống kê tìm kiếm</button><span className="nav-label">QUẢN TRỊ</span><button onClick={()=>openSimple("Cấu hình kho tri thức",["Chu kỳ review mặc định: 12 tháng", "Cảnh báo sớm: trước 30 ngày", "Phân loại đang sử dụng: 5"])}><Icon tone="slate">⚙</Icon>Cấu hình kho tri thức</button><button onClick={()=>openSimple("Phân quyền",["Admin / Leadership: xem toàn công ty", "Knowledge Manager: xem và quản lý review", "Team Lead / PIC: xem theo team được phân quyền"])}><Icon tone="slate">♙</Icon>Phân quyền</button></nav><button className="sidebar-help" onClick={()=>openSimple("Hướng dẫn sử dụng",["Mỗi mục trên sidebar có bộ lọc riêng", "Chọn Đơn vị trước rồi chọn Team", "Nút i hiển thị công thức metric"])}><span>?</span><div><strong>Cần trợ giúp?</strong><small>Xem hướng dẫn sử dụng</small></div></button><button className="profile" onClick={()=>openSimple("Dung Nguyen",["Vai trò: Knowledge Manager", "Phạm vi: Toàn công ty", "Lần truy cập gần nhất: Hôm nay"])}><span>DN</span><div><strong>Dung Nguyen</strong><small>Knowledge Manager</small></div><b>⋯</b></button></aside>
    {sidebarOpen&&<button className="backdrop" aria-label="Đóng menu" onClick={()=>setSidebarOpen(false)}/>}<main><header className="topbar"><button className="menu-button" onClick={()=>setSidebarOpen(true)}>☰</button><div className="page-title"><Icon>{meta.icon}</Icon><div><strong>{meta.title}</strong><span>{meta.subtitle}</span></div></div><div className="top-actions"><label className="search"><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Tìm bài viết, thành viên..."/></label><button className="round-button" aria-label="Trợ giúp" onClick={()=>openSimple("Trợ giúp nhanh",["Mỗi trang lưu bộ lọc độc lập", "Nút i hiển thị công thức metric", "Mọi bảng và biểu đồ đều có thể mở chi tiết"])}>?</button><button className="round-button notification" aria-label="Thông báo" onClick={()=>openSimple("Thông báo",["31 bài cần review gấp", "3 thành viên cần follow-up", "6 bài đang chờ duyệt quá SLA"])}>♢<i/></button><button className="avatar" onClick={()=>openSimple("Tài khoản",["Dung Nguyen", "Knowledge Manager", "Toàn công ty"])}>DN</button></div></header>
    <div className="filter-panel"><div className="filter-block"><div className="filter-caption"><strong>Bộ lọc {meta.title}</strong><span>Được lưu riêng cho mục này</span></div><div className="filters"><SelectField label="Đơn vị" value={unit} values={units} onChange={value=>updateFilter("unit",value)}/><SelectField label="Team" value={team} values={teamOptions} onChange={value=>updateFilter("team",value)}/><SelectField label="Thời gian" value={period} values={periods} onChange={value=>updateFilter("period",value)}/><SelectField label="Phân loại" value={category} values={categories} onChange={value=>updateFilter("category",value)}/></div></div><button className="reset-button" onClick={reset}>↻ Đặt lại</button></div><div className="filter-summary"><span>Đang xem:</span><Badge tone="blue">{unit}</Badge><Badge tone="slate">{team}</Badge><Badge tone="slate">{period}</Badge><Badge tone="slate">{category}</Badge></div>
    {noResults?<div className="content"><EmptyState onReset={reset}/></div>:<div className="content">{tab==="overview"&&<Overview factor={factor} open={setPanel} navigate={navigate}/>} {tab==="articles"&&<Articles factor={factor} open={setPanel} notify={notify} focus={focus}/>} {tab==="members"&&<Members factor={factor} open={setPanel} notify={notify}/>} {tab==="health"&&<Health open={setPanel}/>}</div>}</main>
